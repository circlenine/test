...
ok(P('0904\n0905') === null, '2行あったら対象外');
ok(P('9/4') === null, '区切り付きは対象外（4桁固定にしたため）');
ok(P('9月4日') === null, '「9月4日」も対象外');
ok(P('123456789') === null, '9桁は長すぎるので日付メモとみなさない');
...
has(E('895').why, '4桁', '3桁のときは「4桁で」と言う');
...
console.log('\n■ 案内の文面');
{
  ...
  ok(h.split('\n').length >= 12 && h.length < 500, '長すぎず短すぎない');
}
...
console.log('\n■ 読めない数字を受けたら案内を返信する');
{
  ...
  F('handleDateNote_')(ev, P('895'));
  ok(sent.length === 1, '1通だけ返す');
  has(sent[0], '日付は4桁で固定です', '案内が入っている');
...
ok(ymd(F('parseDateNote_')('251231', new CtxDate(2026, 0, 5, 20, 0, 0))) === '2025-12-31',
